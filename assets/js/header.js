...
const header = {
	...
	handleClickBtnMenu() {
		btnMenuHeader.click(() => {
			...
			overlay.hide();
			if (
				($(window).width() < 1350 || window.location.pathname === '/video-detail.html' || window.location.pathname === '/youtube-base/video-detail.html') &&
				sidebar.hasClass('sidebar--active')
			) {
				overlay.show();
			}

			if (window.location.pathname === '/video-detail.html' || window.location.pathname === '/youtube-base/video-detail.html') {
				sidebar.css({
					top: 0,
				});
				sidebarHeader.css({
